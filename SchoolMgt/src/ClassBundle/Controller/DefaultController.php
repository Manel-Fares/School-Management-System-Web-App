    ...
    public function affetudiclassAction()
{
    ...
    public function getStudentbyclassAction()
    {
        $em=$this->getDoctrine()->getManager();
        $etdds=$em->getRepository('schoolBundle:Users')->findByRoleClass("ROLE_ETUDIANT");
        $classe=$em->getRepository('schoolBundle:Classe')->findAll();


        return $this->render('ClassBundle:Default:getStudent.html.twig',array(
            'etds'=>$etdds,
            'classe'=>$classe
        ));
    }
    public function getTeacherbyclassAction()
    {
        $em=$this->getDoctrine()->getManager();
        $teacher=$em->getRepository('ClassBundle:Classeenseignantmatiere')->findAll();
        $classe=$em->getRepository('schoolBundle:Classe')->findAll();
        return $this->render('ClassBundle:Default:getTeacherclass.html.twig',array(
            'teacher'=>$teacher,
            'classe'=>$classe
        ));
    }
...
if($request->isMethod('post')){
    ...
    $user=$em->getRepository('schoolBundle:Users')->findOneBy(array('id'=>$id+0));
    $mat=$em->getRepository('schoolBundle:Matier')->findOneBy(array('id'=>$idmat+0));

$sujet="you are assigned successfully to the class : ".$classe->getName()." and with subject ".$mat->getNom() ;
...
$em->flush();
    ...
    public function liveSearchAction(Request $request)
    {
        ...
        $absences = $em->getRepository('ClassBundle:Absence')->findAbsenceLiveDQL($name);

        return $this->render('ClassBundle:Default:searchlive.html.twig', array(
            'absences' => $absences,
        ));
    }
    ...
    {
        $em=$this->getDoctrine()->getManager();
        $projets = $em->getRepository('ClassBundle:Events')->findAll();
        $dompdf = new Dompdf();
        $html = $this->renderView('events/indexPdf.html.twig', array(
            'projets' => $projets,
    ...
    public function searchStudentClassAction(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $name= $request->get('dat');
        $use = $em->getRepository('schoolBundle:Users')->findStudentClassDQL($name,"ROLE_ETUDIANT");
        return $this->render('ClassBundle:Default:SearchStudentByClass.html.twig', array(
            'etds' => $use,
        ));
    }
    public function searchTeacherClassAction(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $name= $request->get('dat');
        $teacher = $em->getRepository('schoolBundle:Users')->findTeacherClassDQL($name);
        return $this->render('ClassBundle:Default:SearchTeacherByClass.html.twig', array(
            'teacher' => $teacher,
        ));
    }
