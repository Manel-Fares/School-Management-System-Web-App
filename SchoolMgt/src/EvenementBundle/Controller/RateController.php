...
class RateController extends Controller
{
    ...
    public function new2Action(Request $request,$idclub)
    {
        ...
        $club = $em->getRepository('EvenementBundle:Club')->find($idclub+0);
        //  var_dump($club->getIdclub());
        $user = $this->container->get('security.token_storage')->getToken()->getUser();

        $rateuser=$em->getRepository('EvenementBundle:Club')->testt($idclub,$this->getUser()->getId());
        $form = $this->createForm('EvenementBundle\Form\RateType', $Rate);
        $form->handleRequest($request);
            ...
            if($rateuser!=null){
                for($i=0;$i<count($rateuser);$i++){
                    ...
                    if( $rateuser[$i]->getIduser()->getId()!=$user->getId() ){
                        $Rate->setRating($form['rating']->getData());
                        $Rate->setIduser($user);
                        $Rate->setIdc($club);
                        $em->persist($Rate);
                        $em->flush();
                        return $this->redirectToRoute('club_show_image');

                    }
                    else if( $rateuser[$i]->getIduser()->getId()==$user->getId() ){

                        $rateuser[$i]->setRating($form['rating']->getData());
                        //  var_dump(($form['rating']->getData()));
                        // var_dump( $rateuser[$i]);
