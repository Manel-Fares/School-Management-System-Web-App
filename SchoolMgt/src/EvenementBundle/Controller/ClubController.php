...
class ClubController extends Controller
{
     ...
     * @Method({"GET","POST"})
     */
    public function indexrechAction(Request $request)
    { $em = $this->getDoctrine()->getManager();

        /*if($request ->isMethod('POST')){
            $varr=$request->get('search');
            //  $c = $em->getRepository('EvenementBundle:Club')->search('aa');
            $bb="aaaa";
            $clu = $em->getRepository('EvenementBundle:Club')->search("aaa");
            var_dump($varr);
            $clubs=$em->getRepository('EvenementBundle:Club')->find(1);
            return $this->render('club/indexrech.html.twig', array(
                'club' => $clubs,
            ));
        ...
        $club = $em->getRepository('EvenementBundle:Club')->search($varr);

        $aa=array();
        for($i=0;$i<count($club);$i++){

            $clubs=$em->getRepository('EvenementBundle:Club')->findby(array('idclub'=>$club[$i]['idclub']));
        ...
        //  $c=$em->getRepository('EvenementBundle:Club')->find(1);
        //var_dump($aa);
        return $this->render('club/indexrech.html.twig', array(
            'clubs' => $aa,
        ));
...
//var_dump($clubs);
    ...
    public function showAction(Club $club)
    {
        ...
        if ($user->hasRole('ROLE_PERSONNEL')) {
            ...
            ));}else{ return $this->render('club/showClubResponsableDetail.html.twig', array(
            'club' => $club,
            'delete_form' => $deleteForm->createView(),'clubs' => $clubs,'evenements'=>$evenements,'bd'=>$bd,'ct'=>$ct
        ));}
    }
     ...
     * Finds and displays a club entity.
     *
     * @Route("/{id}", name="testResponsable")
     * @Method("GET")
     */
    public function TestAction(Users $user)
    {
        $em = $this->getDoctrine()->getManager();
        $clubs = $em->getRepository('EvenementBundle:Club')->findOneBy(array('idresponsable'=>$user));
        // var_dump($clubs);
        return $this->render('club/show.html.twig', array(
            'clubs' => $clubs,
        ));
    ...
    public function editAction(Request $request, Club $club)
    {
        ...
        $editFormResp = $this->createForm('EvenementBundle\Form\ClubType', $club);
        $editFormResp->handleRequest($request);
       // $editFormResp->add('idresponsable',HiddenType::class);
        if ($editForm->isSubmitted() && $editForm->isValid()) {
            if($editForm['file']->getData()!=null){$club->UploadProfilePicture();
            }
            ...
            if ($user->hasRole('ROLE_PERSONNEL')) {
                ...
                return $this->redirectToRoute('clubResponsable');
            }
        }
        ...
        if ($user->hasRole('ROLE_PERSONNEL')) {
            ...
            return $this->render('club/edit2.html.twig', array(
                'club' => $club,
                'edit_form' => $editFormResp->createView(),
                'delete_form' => $deleteForm->createView(),
            ));}
    }
     ...
     * Finds and displays a club entity.
     *
     * @Route("/clubresponsable/", name="clubResponsable")
     * @Method("GET")
     */
    public function ClubResponsableAction()
    {
        $user = $this->container->get('security.token_storage')->getToken()->getUser();

        $em = $this->getDoctrine()->getManager();
        $club = $em->getRepository('EvenementBundle:Club')->findBy(array('idresponsable' => $user));
        ...
        $evenements= $em->getRepository('EvenementBundle:Evenement')->nbrEvenementTotale();
        // var_dump($clubs);
        return $this->render('club/showClubResponsable.html.twig', array(
            'clubs' => $aa,'evenements'=>$evenements,
        ));
    }
