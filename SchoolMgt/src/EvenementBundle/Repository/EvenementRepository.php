...
class EvenementRepository extends  \Doctrine\ORM\EntityRepository
{
    ...
    public function EvenementProch(){
        ...
        $qry=$this->getEntityManager()
            ->createQuery("SELECT m.idevenement,  m.datedebut, m.datefin,m.image  FROM EvenementBundle:Evenement m where DATE_DIFF(m.datefin,:date)  < 7 OR  DATE_DIFF(m.datefin,:date) = 7")
            ->setParameter('date', $date->format('Y-m-d'));
