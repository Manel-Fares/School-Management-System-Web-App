...
class DefaultController extends Controller
{
    ...
    public function calculerResAction(){
        ...
        $subject='School Result';

        $adminmail='manelfares5@gmail.com';


        $message = (new \Swift_Message($subject))
            ->setFrom('manelfares5@gmail.com')
            ->setTo($adminmail)
            ->setBody($msg)
        ;
        $mailer->send($message);
...
******************************* omar *****************************/
    ...
    public function addRatingAction($rating,$idc,$idu)
    {
        $em = $this->getDoctrine()->getManager();
        $Rate = new Rate();;
        $rateuser=$em->getRepository('EvenementBundle:Club')->testt($idc,$idu);

        // $rateuser=$em->getRepository('EvenementBundle:Rate')->find($idu);
        $user=$em->getRepository('schoolBundle:Users')->findOneBy(array('id'=>$idu));
        ...
        if($rateuser!=null){
            for($i=0;$i<count($rateuser);$i++){
                ...
                else if( $rateuser[$i]->getIduser()->getId()==$user->getId() ){
                    //   var_dump($user);
                    $rateuser[$i]->setRating($rating);
                    $em->persist($rateuser[$i]);
                    $em->flush();
                    $serializer = new Serializer([new ObjectNormalizer()]);
                    $formatted = $serializer->normalize($rateuser[$i]);
                    return new JsonResponse($formatted);
    ...
    */
    public function addParticipationAction($ide,$idu)
    {
        ...
        $rateuser=$em->getRepository('EvenementBundle:Participation')->testt($ide,$idu);
        ...
        if($rateuser!=null){
            for($i=0;$i<count($rateuser);$i++){
                ...
                else if( $rateuser[$i]->getIduser()->getId()==$user->getId() &&$rateuser[$i]->getIdevenement()->getIdevenement() ==$evenemnt->getIdevenement()){
    ...
    public function ClubResponsabletestAction($idu)
    {$aa=array();
        $em = $this->getDoctrine()->getManager();
        $responsable=$em->getRepository('EvenementBundle:Club')->findOneBy(array('idresponsable'=>$idu));
        array_push($aa,$responsable);
    ...
    public function ClubResponsableAction($idu)
    {
        ...
        for ($i = 0; $i < count($club); $i++){
            $clubs = $em->getRepository('EvenementBundle:Club')->nbrEvenementClub($club[$i]->getIdclub());
            $bd = $em->getRepository('EvenementBundle:Demandeevenement')->budgetEvenementClub($club[$i]->getIdclub());
            $rate=$em->getRepository('EvenementBundle:Club')
                ->ClubRate($club[$i]->getIdclub());
            for ($j = 0; $j < count($evenements); $j++) {
                for ($k = 0; $k < count($rate); $k++) {
                    foreach ($clubs as $cl) {


                        $aa[$i]['x'] = $cl['x'] / $evenements[$j]['nbr'];
                        $aa[$i]['nomclub'] = $cl['nomclub'];
                        $aa[$i]['idclub'] = $cl['idclub'];
                        $aa[$i]['domaine'] = $cl['domaine'];
                        $aa[$i]['rating'] = $rate[$k]['rating'];
    ...
    public function  ClubEditAction($idclb,$img,$d)
    {

        $em = $this->getDoctrine()->getManager();
        $club = $em->getRepository('EvenementBundle:Club')->find($idclb);
        if($img!="null" && $d=="null"){$club->setImage($img);}
        else if($d!="null" && $img=="null"){ $club->setDomaine($d);}
        else{$club->setImage($img);
            $club->setDomaine($d);}
    ...
    public function afficherEvenementClubAction($idu)
    {
        $em = $this->getDoctrine()->getManager();
        ...
        $evenements=array();
        $club=$em->getRepository('EvenementBundle:Club')->findBy(array('idresponsable'=>$idu));
        for($j=0;$j<count($club);$j++){
            $evenement = $em->getRepository('EvenementBundle:Evenement')->afficerSpecifiqueClub($club[$j]->getidClub());
            for($i=0;$i<count($evenement);$i++){
                //   var_dump($evenements[$i]['idevenement']);
        ...
        for($i=0;$i<count($evenements);$i++){
            $evenements[$i]['datedebut']=$evenements[$i]['datedebut']->format('Y-m-d');
            $evenements[$i]['datefin']=$evenements[$i]['datefin']->format('Y-m-d');
        }
        $serializer = new Serializer([new ObjectNormalizer()]);
