    ...
    public function affetudiclassAction()
{
    ...
    $etdds=$em->getRepository('EvenementBundle:Users')->findByRole("ROLE_ETUDIANT");
    $classe=$em->getRepository('ClassBundle:Classe')->findAll();
...
if($request->isMethod('post')){
    $idd=$request->get('data')+0;
        $classe=$em->getRepository('ClassBundle:Classe')->find($idd);

    $user=$em->getRepository('EvenementBundle:Users')->find($id+0);
...
$cnt=count($absences);
    ...
    public function getEmploisAction()
    {
        ...
        $Userr = $em->getRepository('EvenementBundle:Users')->findBy(array('id'=>$user));
